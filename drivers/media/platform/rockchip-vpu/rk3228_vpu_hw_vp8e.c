...
/**
 * struct rk3228_vpu_vp8e_ctrl_buf - hardware control buffer layout
 ...
 */
struct rk3228_vpu_vp8e_ctrl_buf {
	u32 ext_hdr_size;
	u32 dct_size;
	u8 rsvd[1016];
};
...
int rk3228_vpu_vp8e_init(struct rockchip_vpu_ctx *ctx)
{
	...
	ret = rockchip_vpu_aux_buf_alloc(vpu, &ctx->hw.vp8e.ctrl_buf,
				sizeof(struct rk3228_vpu_vp8e_ctrl_buf));
	if (ret) {
	...
	ref_buf_size = ref_luma_size(width, height) * 3 / 2 * 2;
...
static inline u32 enc_in_img_ctrl(struct rockchip_vpu_ctx *ctx)
{
	...
	const struct rockchip_vp8e_reg_params_new *params =
		ctx->run.vp8e.reg_params_new;
...
static void rk3228_vpu_vp8e_set_buffers(struct rockchip_vpu_dev *vpu,
					struct rockchip_vpu_ctx *ctx)
{
	...
	if (params->is_intra)
		to_vb2_v4l2_buffer(&ctx->run.dst->b.vb2_buf)->flags |=
...
static int32_t cost_mv(int32_t mvd, const int32_t *mv_prob)
{
	int32_t i, tmp, bit_cost = 0;
...
static void rk3228_vpu_vp8e_set_params(struct rockchip_vpu_dev *vpu,
				       struct rockchip_vpu_ctx *ctx)
{
	const struct rockchip_vp8e_reg_params_new *params =
		ctx->run.vp8e.reg_params_new;
	...
	reg = VEPU_REG_PENALTY_4X4MV(MIN(511, (8 * split_penalty[qp] + 500) / 16))
		| VEPU_REG_ZERO_MV_FAVOR_D2(0);
	vepu_write_relaxed(vpu, reg, VEPU_REG_MVC_RELATE);

	/* initialize quant table for segment0 */
	deq = dc_q_lookup[qp];
	reg = VEPU_REG_VP8_SEG0_QUT_DC_Y1(MIN((1 << 16) / deq, 0x3fff));
	reg |= VEPU_REG_VP8_SEG0_ZBIN_DC_Y1(((qzbin_factors[qp] * deq) + 64) >> 7);
	reg |= VEPU_REG_VP8_SEG0_RND_DC_Y1((qrounding_factors[qp] * deq) >> 7);
	vepu_write_relaxed(vpu, reg, VEPU_REG_VP8_SEG0_QUANT_DC_Y1);

	deq = ac_q_lookup[qp];
	reg = VEPU_REG_VP8_SEG0_QUT_AC_Y1(MIN((1 << 16) / deq, 0x3FFF));
	reg |= VEPU_REG_VP8_SEG0_ZBIN_AC_Y1(((qzbin_factors[qp] * deq) + 64) >> 7);
	...
	reg = VEPU_REG_VP8_SEG0_QUT_DC_Y2(MIN((1 << 16) / deq, 0x3FFF));
	reg |= VEPU_REG_VP8_SEG0_ZBIN_DC_Y2((qzbin_factors[qp] * deq + 64) >> 7);
	...
	reg = VEPU_REG_VP8_SEG0_QUT_AC_Y2(MIN((1 << 16) / deq, 0x3FFF));
	reg |= VEPU_REG_VP8_SEG0_ZBIN_AC_Y2((qzbin_factors[qp] * deq + 64) >> 7);
	...
	reg = VEPU_REG_VP8_SEG0_QUT_DC_CHR(MIN((1 << 16) / deq, 0x3FFF));
	reg |= VEPU_REG_VP8_SEG0_ZBIN_DC_CHR((qzbin_factors[qp] * deq + 64) >> 7);
	reg |= VEPU_REG_VP8_SEG0_RND_DC_CHR((qrounding_factors[qp] * deq) >> 7);
	vepu_write_relaxed(vpu, reg, VEPU_REG_VP8_SEG0_QUANT_DC_CHR);

	deq = ac_q_lookup[qp];
	reg = VEPU_REG_VP8_SEG0_QUT_AC_CHR(MIN((1 << 16) / deq, 0x3FFF));
	reg |= VEPU_REG_VP8_SEG0_ZBIN_AC_CHR((qzbin_factors[qp] * deq + 64) >> 7);
	...
	for (i = 0; i < 3; i++) {
		deq = dc_q_lookup[qp];
		reg = VEPU_REG_VP8_QUT_DC_Y1(MIN((1 << 16) / deq, 0x3fff));
		deq = dc_q_lookup[qp] * 2;
		reg |= VEPU_REG_VP8_QUT_DC_Y2(MIN((1 << 16) / deq, 0x3FFF));
		vepu_write_relaxed(vpu, reg, VEPU_REG_VP8_QUT_1ST(i));

		deq = MIN(dc_q_lookup[qp], 132);
		reg = VEPU_REG_VP8_QUT_DC_CHR(MIN((1 << 16) / deq, 0x3FFF));
		deq = ac_q_lookup[qp];
		reg |= VEPU_REG_VP8_QUT_AC_Y1(MIN((1 << 16) / deq, 0x3FFF));
		vepu_write_relaxed(vpu, reg, VEPU_REG_VP8_QUT_2ND(i));

		deq = ac_q_lookup[qp] * 155 / 100;
		reg = VEPU_REG_VP8_QUT_AC_Y2(MIN((1 << 16) / deq, 0x3FFF));
		deq = ac_q_lookup[qp];
		reg |= VEPU_REG_VP8_QUT_AC_CHR(MIN((1 << 16) / deq, 0x3FFF));
		vepu_write_relaxed(vpu, reg, VEPU_REG_VP8_QUT_3RD(i));

		deq = dc_q_lookup[qp];
		reg = VEPU_REG_VP8_QUT_ZB_DC_Y1((qzbin_factors[qp] * deq + 64) >> 7);
		deq = dc_q_lookup[qp] * 2;
		reg |= VEPU_REG_VP8_QUT_ZB_DC_Y2((qzbin_factors[qp] * deq + 64) >> 7);
		deq = MIN(dc_q_lookup[qp], 132);
		reg |= VEPU_REG_VP8_QUT_ZB_DC_CHR((qzbin_factors[qp] * deq + 64) >> 7);
		vepu_write_relaxed(vpu, reg, VEPU_REG_VP8_QUT_4TH(i));

		deq = ac_q_lookup[qp];
		reg = VEPU_REG_VP8_QUT_ZB_AC_Y1((qzbin_factors[qp] * deq + 64) >> 7);
		deq = ac_q_lookup[qp] * 155 / 100;
		reg |= VEPU_REG_VP8_QUT_ZB_AC_Y2((qzbin_factors[qp] * deq + 64) >> 7);
		deq = ac_q_lookup[qp];
		reg |= VEPU_REG_VP8_QUT_ZB_AC_CHR((qzbin_factors[qp] * deq + 64) >> 7);
		vepu_write_relaxed(vpu, reg, VEPU_REG_VP8_QUT_5TH(i));

		deq = dc_q_lookup[qp];
		reg = VEPU_REG_VP8_QUT_RND_DC_Y1((qrounding_factors[qp] * deq) >> 7);
		deq = dc_q_lookup[qp] * 2;
		reg |= VEPU_REG_VP8_QUT_RND_DC_Y2((qrounding_factors[qp] * deq) >> 7);
		deq = MIN(dc_q_lookup[qp], 132);
		reg |= VEPU_REG_VP8_QUT_RND_DC_CHR((qrounding_factors[qp] * deq) >> 7);
		vepu_write_relaxed(vpu, reg, VEPU_REG_VP8_QUT_6TH(i));

		deq = ac_q_lookup[qp];
		reg = VEPU_REG_VP8_QUT_RND_AC_Y1((qrounding_factors[qp] * deq) >> 7);
		deq = ac_q_lookup[qp] * 155 / 100;
		reg |= VEPU_REG_VP8_QUT_RND_AC_Y2((qrounding_factors[qp] * deq) >> 7);
		deq = ac_q_lookup[qp];
		reg |= VEPU_REG_VP8_QUT_RND_AC_CHR((qrounding_factors[qp] * deq) >> 7);
		vepu_write_relaxed(vpu, reg, VEPU_REG_VP8_QUT_7TH(i));

		reg = VEPU_REG_VP8_DEQUT_DC_Y1(dc_q_lookup[qp]);
		reg |= VEPU_REG_VP8_DEQUT_DC_Y2(dc_q_lookup[qp] * 2);
		reg |= VEPU_REG_VP8_DEQUT_DC_CHR(MIN(dc_q_lookup[qp], 132));
		reg |= VEPU_REG_VP8_SEG_FILTER_LEVEL(params->filter_level);//26
		vepu_write_relaxed(vpu, reg, VEPU_REG_VP8_QUT_8TH(i));

		reg = VEPU_REG_VP8_DEQUT_AC_Y1(ac_q_lookup[qp]);
		reg |= VEPU_REG_VP8_DEQUT_AC_Y2(ac_q_lookup[qp] * 155 / 100);
		reg |= VEPU_REG_VP8_DEQUT_AC_CHR(ac_q_lookup[qp]);
		vepu_write_relaxed(vpu, reg, VEPU_REG_VP8_QUT_9TH(i));
	}
#endif
	vepu_write_relaxed(vpu, params->bool_enc_value, VEPU_REG_VP8_BOOL_ENC_VALUE);
	...
	tmp = qp * 2 + 8;
	for (i = 0; i < 5; i++) {
		reg = VEPU_REG_VP8_INTRA_4X4_PENALTY_0((intra_4_tree_penalty[i * 2] * tmp) >> 8);
		reg |= VEPU_REG_VP8_INTRA_4x4_PENALTY_1((intra_4_tree_penalty[i * 2 + 1] * tmp) >> 8);
		vepu_write_relaxed(vpu, reg, VEPU_REG_VP8_INTRA_4X4_PENALTY(i));
	}

	/* Intra 16x16 mode */
	tmp = qp * 2 + 64;
	for (i = 0; i < 2; i++) {
		reg = VEPU_REG_VP8_INTRA_16X16_PENALTY_0((intra_16_tree_penalty[2 * i] * tmp) >> 8);
		reg |= VEPU_REG_VP8_INTRA_16X16_PENALTY_1((intra_16_tree_penalty[2 * i + 1] * tmp) >> 8);
		vepu_write_relaxed(vpu, reg, VEPU_REG_VP8_INTRA_16X16_PENALTY(i));
	}
...
{
	const struct rockchip_vp8e_reg_params_new *params = ctx->run.vp8e.reg_params_new;
	struct rockchip_vpu_dev *vpu = ctx->dev;
	u32 reg;
...
{
	struct rk3228_vpu_vp8e_ctrl_buf *ctrl_buf = ctx->hw.vp8e.ctrl_buf.cpu;
	const struct rockchip_vp8e_reg_params_new *params = ctx->run.vp8e.reg_params_new;
