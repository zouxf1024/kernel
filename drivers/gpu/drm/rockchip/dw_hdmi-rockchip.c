				  ...
				  const struct dw_hdmi_plat_data *plat_data)
{
	...
	struct platform_device *pdev =
			container_of(hdmi->dev, struct platform_device, dev);
	struct resource *iores;
	struct reset_control *ctrl_rst;
	int ret;

	ctrl_rst = devm_reset_control_get(hdmi->dev, "phy");
	reset_control_assert(ctrl_rst);
	usleep_range(10, 20);
	reset_control_deassert(ctrl_rst);

	ctrl_rst = devm_reset_control_get(hdmi->dev, "ctrl");
	reset_control_assert(ctrl_rst);
	usleep_range(10, 20);
	reset_control_deassert(ctrl_rst);
