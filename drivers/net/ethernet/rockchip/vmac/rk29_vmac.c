...
int vmac_open(struct net_device *dev)
{
	...
	err = clk_set_rate(pdata->clk_mac_ref_div, 50000000);
	if (err)
		pr_err("Fail to set clk_mac_ref_div %d\n", err);

	err = clk_set_rate(pdata->clk_mac_pll_div, 25000000);
	if (err)
		pr_err("Fail to set clk_mac_pll_div %d\n", err);

	pr_info("%s -- clk_mac_ref_div = %ld hz, clk_mac_pll_div = %ld hz\n",
		__func__,
		clk_get_rate(pdata->clk_mac_ref_div),
		clk_get_rate(pdata->clk_mac_pll_div));
...
extern struct device_node *vmac_node;
