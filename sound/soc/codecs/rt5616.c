...
#define VERSION "0.1.3 alsa 1.0.24"
...
static struct rt5616_init_reg init_list[] = {
	...
	{RT5616_HP_VOL		, 0x9898}, //unmute HPVOL
	{RT5616_OUT_L3_MIXER	, 0x0378}, //DACL1 -> OUTMIXL
	{RT5616_OUT_R3_MIXER	, 0x0378}, //DACR1 -> OUTMIXR
	/*LOUT*/
	{RT5616_LOUT_MIXER	, 0x3000},
	/*Capture*/
	{RT5616_REC_L2_MIXER	, 0x00dd}, //MIC1 -> RECMIXL
	{RT5616_REC_R2_MIXER	, 0x00dd}, //MIC1 -> RECMIXR
	...
	{RT5616_ADC_BST_VOL	, 0xf000}, //Set ADC digital boost gain to max
	/*MIC*/
...
static const struct reg_default rt5616_reg[] = {
	...
	{ RT5616_DRC_AGC_1,		0x2206 },
	{ RT5616_DRC_AGC_2,		0x1f00 },
...
static bool rt5616_volatile_register(
...
static bool rt5616_readable_register(
	struct device *dev, unsigned int reg)
{
	switch (reg) {
	case RT5616_RESET:
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[1] = snd_soc_read(component->codec, regctl_addr);
	return 0;
}
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if(ucontrol->value.integer.value[1] <= REGVAL_MAX)
		snd_soc_write(component->codec, regctl_addr, ucontrol->value.integer.value[1]);
	return 0;
}
#endif
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	return snd_soc_update_bits(component->codec, mc->reg, RT5616_L_VOL_MASK |
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
#ifdef EQ_FUNC
		rt5616_update_eqmode(codec,LOUT);
#endif
		hp_amp_power(codec,1);
		snd_soc_update_bits(codec, RT5616_PWR_ANLG1,
...
static int rt5616_dac1_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	switch (event) {
	case SND_SOC_DAPM_PRE_PMD:
#ifdef EQ_FUNC
		rt5616_update_eqmode(snd_soc_dapm_to_codec(w->dapm), NORMAL);
#endif
		break;
	default:
		return 0;
...
static const struct snd_soc_dapm_widget rt5616_dapm_widgets[] = {
	...
	SND_SOC_DAPM_DAC_E("DAC L1", NULL, RT5616_PWR_DIG1,
			RT5616_PWR_DAC_L1_BIT, 0, rt5616_dac1_event,
			SND_SOC_DAPM_PRE_PMD),
	SND_SOC_DAPM_DAC_E("DAC R1", NULL, RT5616_PWR_DIG1,
			RT5616_PWR_DAC_R1_BIT, 0, rt5616_dac1_event,
			SND_SOC_DAPM_PRE_PMD),
	/* OUT Mixer */
...
static int rt5616_probe(struct snd_soc_codec *codec)
{
	...
	*/
	...
	ioctl_ops->index_write = rt5616_index_write;
	ioctl_ops->index_read = rt5616_index_read;
	ioctl_ops->index_update_bits = rt5616_index_update_bits;
	ioctl_ops->ioctl_common = rt5616_ioctl_common;
...
static const struct regmap_config rt5616_regmap = {
	...
	.val_bits = 16,
	.use_single_rw = true,
	//.max_register = RT5616_VENDOR_ID2 + 1 + (ARRAY_SIZE(rt5645_ranges) *
	//				       RT5645_PR_SPACING),
		    ...
		    const struct i2c_device_id *id)
{
	...
	regmap = devm_regmap_init_i2c(i2c, &rt5616_regmap);
	if (IS_ERR(regmap)) {
		ret = PTR_ERR(regmap);
...
static int rt5616_modinit(void)
{
	printk("rt5616: enter %s\n",__func__);
...
static void rt5616_modexit(void)
{
